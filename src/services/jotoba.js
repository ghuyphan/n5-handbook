...
function renderOfflineState() {
    const offlineIcon = `<svg class="w-16 h-16 text-secondary opacity-60 mb-4 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>`;

    const offlineHTML = `
        <div class="search-placeholder-box text-center content-anim-fade-in">
            ${offlineIcon}
            <h3 class="text-xl font-semibold text-primary">${getUIText('offlineTitle')}</h3>
            <p class="text-secondary text-base mt-1 max-w-md mx-auto">${getUIText('offlineSearchMessage')}</p>
        </div>`;
    ...
    if (placeholderContainer) {
        ...
        placeholderContainer.innerHTML = offlineHTML;
    }
}
...
function renderErrorState(error, query) {
    ...
    errorMessage = getUIText(errorMessages[errorKey]);
    ...
    const errorContentHTML = `
        <div class="search-placeholder-box text-center content-anim-fade-in">
            ${errorIcon}
            <h3 class="text-xl font-semibold text-primary">${getUIText('searchErrorTitle')}</h3>
...
async function handleExternalSearchInternal(query, forceRefresh = false, isTabSwitch = false) {
    ...
    // Check if offline - show friendly message
    if (!navigator.onLine) {
        renderOfflineState();
        return;
    }
