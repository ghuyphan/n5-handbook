...
async function init() {
    try {
        ...
        setTimeout(() => {
            document.querySelector(`.level-switch button[data-level="${state.currentLevel}"]`)?.classList.add('active');
            document.querySelectorAll('.lang-switch').forEach(moveLangPill);
...
// Expose functions to the global scope for inline HTML `onclick` attributes
