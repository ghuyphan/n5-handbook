...
// --- Database Setup ---
const dbPromise = openDB('HandbookDB', 1, {
    upgrade(db) {
...
function setLanguage(lang, skipRender = false) {
    ...
    const processText = (textKey) => {
        ...
        if (text.includes('{level}')) {
            text = text.replace('{level}', currentLevel.toUpperCase());
        }
        return text;
    };
...
/**
 * Synchronously syncs the theme toggle switches with the
...
/**
 * Toggles the theme, applies the class to the root <html> element,
...
function changeTab(tabName, buttonElement) {
    ...
    if (targetButton) {
        ...
        if (isMobileView) {
            ...
            const titleText = (appData.ui?.[currentLang]?.[titleKey]) || titleSpan?.textContent || '';
...
function updatePinButtonState(activeTabId) {
    ...
    if (activeTabId && activeTabId === pinnedTab) {
...
function setupFuseForTab(tabId) {
    if (fuseInstances[tabId] || !appData[tabId]) return;
...
function updateProgressDashboard() {
    const overviewContainer = els.progressOverview;
    const progressTabContainer = els.progressTab;
...
function renderContent() {
    ...
    renderSafely(() => {
        if (els.hiraganaTab) {
            els.hiraganaTab.innerHTML = createStaticSection(prepareGojuonData(appData.hiragana), '🌸', 'var(--accent-pink)');
    ...
    renderSafely(() => {
        if (els.katakanaTab) {
            els.katakanaTab.innerHTML = createStaticSection(prepareGojuonData(appData.katakana), '🤖', 'var(--accent-blue)');
            setupFuseForTab('katakana');
        }
    });

    renderSafely(() => {
        if (!appData.keyPoints || !els.keyPointsTab) return;
        let keyPointsHTML = '';
    ...
    renderSafely(() => {
        if (!appData.grammar || !els.grammarContainer) {
            if (els.grammarContainer) els.grammarContainer.innerHTML = '';
            return;
        }
        let grammarHTML = '';
        ...
        if (grammarTab) {
            grammarTab.innerHTML = `<div class="space-y-4">${grammarHTML}</div>`;
        }
...
function setupImportModal() {
    ...
    const updateModalLocale = () => {
        els.importModal.querySelectorAll('[data-lang-key]').forEach(el => {
            const key = el.dataset.langKey;
            const replacements = (key === 'importButton' && els.importBtn.disabled) ? { status: '...' } : {};
            el.textContent = getUIText(key, replacements);
        });
        els.importModal.querySelectorAll('[data-lang-placeholder-key]').forEach(el => {
    ...
    const updateImportButtonState = () => {
        ...
        const importButtonText = els.importBtn.querySelector('span');
        if (importButtonText) {
    ...
    const handleFolderSelect = async (files) => {
        ...
        if (selectedFiles.length === 0) {
            els.fileImportArea.innerHTML = `<p class="text-red-400 text-sm">${getUIText('errorNoFolderSelected')}</p>`;
            updateImportButtonState();
            return;
        }
    ...
    els.modalWrapper?.addEventListener('click', (event) => {
        if (event.target === els.modalWrapper) closeModal();
    });

    els.levelNameInput?.addEventListener('input', updateImportButtonState);
    els.importBtn?.addEventListener('click', handleConfirm);

    els.fileImportArea?.addEventListener('click', () => {
        if (!els.fileImportArea.classList.contains('state-preview')) {
            els.fileInput.click();
        }
    });
    els.fileInput?.addEventListener('change', (e) => handleFolderSelect(e.target.files));

    els.fileImportArea?.addEventListener('dragover', (e) => {
        e.preventDefault();
        els.fileImportArea.classList.add('drag-active');
    });
    els.fileImportArea?.addEventListener('dragleave', () => els.fileImportArea.classList.remove('drag-active'));
    els.fileImportArea?.addEventListener('drop', (e) => {
        e.preventDefault();
        els.fileImportArea.classList.remove('drag-active');
...
function setupEventListeners() {
    document.body.addEventListener('click', (e) => {
        const jumpToAction = e.target.closest('[data-action="jump-to-section"]');
        if (jumpToAction) {
            jumpToSection(jumpToAction.dataset.tabName, jumpToAction.dataset.sectionKey);
        }
    });
...
async function init() {
    try {
        ...
        setTimeout(() => {
            const activeLevelButton = document.querySelector(`.level-switch button[data-level="${currentLevel}"]`);
            if (activeLevelButton) activeLevelButton.classList.add('active');

            const activeLangButton = document.querySelector(`.lang-switch button[data-lang="${currentLang}"]`);
...
// --- Make functions available on the window object for inline HTML onclick attributes ---
